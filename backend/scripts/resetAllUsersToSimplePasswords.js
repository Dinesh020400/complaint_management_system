...
async function resetAllPasswords() {
  try {
    ...
    const specificPasswords = {
      'uma@gmail.com': 'asdfgh',
      'dinesh@gmail.com': '147852',
      'admin@gmail.com': 'admin123',
      'user@test.com': 'password123',
      'pavi@gmail.com': 'pavi123',
      'pavi1@gmail.com': 'pavi1123'
    };
    
    // Reset password for each user
    for (const user of users) {
      // Set new password - either from specific passwords or use email username
      let newPassword;
      
      if (specificPasswords[user.email]) {
        newPassword = specificPasswords[user.email];
      } else {
        // Extract username from email
        const emailUsername = user.email.split('@')[0];
        newPassword = emailUsername;
      }
    ...
    for (const user of users) {
      let password;
      if (specificPasswords[user.email]) {
        password = specificPasswords[user.email];
      } else {
